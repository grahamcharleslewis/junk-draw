...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference("Target.count") do
      post targets_url, params: { target: { klass_attribute: @target.klass_attribute, klass_name: @target.klass_name, target: @target.target, week_id: @target.week_id } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { klass_attribute: @target.klass_attribute, klass_name: @target.klass_name, target: @target.target, week_id: @target.week_id } }
